...
列识别规则：
  ...
  - 有 source_name 列则构造 source_ref.source_name，否则用 --source-name（默认 csv_import）
  - source_doi / source_url / source_pmid / source_accession 列若存在则写入 source_ref
  - extraction_confidence 列若存在则覆盖默认置信度（0.85）

执行示例：
  python scripts/io/csv_to_json.py --input data.csv --out records.json \
      --task-id T1 --source-name "user_upload"
"""
import csv
import os
import sys
...
def main():
    parser = setup_cli("csv_to_json", "CSV 转 DataRecord JSON 列表")
    parser.add_argument("--task-id", default="default", dest="task_id", help="任务 ID")
    parser.add_argument("--source-name", default="csv_import", dest="source_name",
                        help="默认 source_name（当 CSV 无 source_name 列时使用）")
    args = parser.parse_args()
    try:
        records = convert(args.input, args.task_id, args.source_name)

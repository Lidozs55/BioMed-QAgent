#!/usr/bin/env python3
"""geo_soft_parser.py — GEO SOFT 格式解析器。
...
示例：
    python geo_soft_parser.py --input GSE12345.family.soft.gz --out result.json
...
def main():
    parser = setup_cli("geo_soft_parser", "GEO SOFT 格式解析器")
    ...
    try:
        ...
        write_output(make_error(f"GEO SOFT 解析失败: {e}"), args.out)
        sys.exit(1)

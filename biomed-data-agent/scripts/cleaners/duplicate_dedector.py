...
执行示例：
  python scripts/cleaners/duplicate_dedector.py --input results/normalized.json --out results/deduped.json
"""
import json
import os
import sys
...
def main():
    parser = setup_cli("duplicate_dedector", "重复检测与去重：保留高置信度记录，检测数值冲突")
    args = parser.parse_args()
    try:
        records = load_records(args.input)
        deduped, report = deduplicate(records)
        # 单独保存 duplicate_report.json
        rep_path = Path(args.out).parent / "duplicate_report.json"
        ...
        print(json.dumps({"status": "ok", "count": len(deduped),
                          "duplicates": report["total_duplicates"],
                          "conflicts": report["total_conflicts"]}, ensure_ascii=False))

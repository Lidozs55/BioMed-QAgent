...
from _base import load_records, save_records, make_field_mapping, setup_cli, statistics
...
def main():
    parser = setup_cli("field_aligner", "字段对齐引擎：统一多源 DataRecord 字段名与值")
    args = parser.parse_args()
    try:
        records = load_records(args.input)
        field_dict = load_dictionaries(args.dictionaries)
        cleaned, field_mapping = align_records(records, field_dict)
        # 单独保存 field_mapping.json（符合 field_mapping.schema.yaml 的列表）
        fm_path = Path(args.out).parent / "field_mapping.json"
        ...
        result = {
            ...
            "count": len(cleaned),
            "stats": statistics(cleaned),
            "field_mapping_path": str(fm_path),
        }
        save_records(result, args.out)
        print(json.dumps({"status": "ok", "count": len(cleaned),
                          "field_mapping_count": len(field_mapping),
                          "stats": statistics(cleaned)}, ensure_ascii=False))

...
用法:
    python scripts/datasources/arxiv_client.py --query "pancreatic cancer" --max 20 --out result.json
    python scripts/datasources/arxiv_client.py --query "Jianpi Sanjie" --max 10
"""
...
def main() -> None:
    parser = setup_cli("arxiv_client", "arXiv 预印本论文检索")
    args = parser.parse_args()
    if not args.query:
        emit_error("缺少 --query 参数")
        sys.exit(1)
    try:
        records = search_arxiv(args.query, args.max, args.task_id)
        write_output(records, args.out)
        log_stderr(f"arxiv: 返回 {len(records)} 条")
    except Exception as e:
        emit_error(f"arxiv 检索失败: {e}")
        sys.exit(1)

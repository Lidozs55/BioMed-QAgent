...
// Tooltip removed — capability icons use native title + aria-label instead
// to avoid nested interactive controls inside model option buttons
...
interface SettingsPanelProps {
  ...
  onClose: () => void
  onFetchModels: (query?: string, baseUrl?: string) => Promise<void>
}
...
export function SettingsPanel(props: SettingsPanelProps) {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      // Models are refreshed by updateSettings (via refreshModels()).
      // No verification GET needed — the save POST itself validates credentials.
      setDirty(false)
      onClose()
    } catch (err) {
  ...
  }, [baseUrl, apiKeyInput, modelName, maxTokens, temperature, topP, repPenalty, enableSearch, thinkingMode, models, initialSettings, onSave, onClose])

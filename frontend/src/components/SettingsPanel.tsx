...
export function SettingsPanel({
  ...
  const handleSave = useCallback(async () => {
    ...
    setDirty(false)
  }, [baseUrl, apiKeyInput, modelName, maxTokens, temperature, topP, repPenalty, enableSearch, thinkingMode, initialSettings, onSave])
                  ...
                  <>
                    <button type="button" id="settings-model"
                      ...
                      {modelsLoading ? (
                        ...
                        <span>{(selectedModel?.name ?? modelName) || "选择模型"}</span>
                      )}

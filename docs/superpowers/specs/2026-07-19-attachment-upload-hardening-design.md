...
5. Keep the current main-branch limits: at most 10 files per request, at most
   500 MiB per file, and at most 2 GiB total.
...
- 413 when a file exceeds 500 MiB or the request exceeds 2 GiB total.
...
3. Stream each upload into that directory in 64 KiB chunks while enforcing the
   500 MiB per-file and 2 GiB total limits.
...
- Files over 500 MiB, totals over 2 GiB, and selections over 10 files show a visible error and are
  not submitted.

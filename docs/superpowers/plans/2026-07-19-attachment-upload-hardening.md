...
- Keep at most 10 files per import request and at most 10 MiB per file.
...
- Constants: `MAX_IMPORT_FILES = 10`, `MAX_IMPORT_FILE_BYTES = 10 * 1024 * 1024` in the frontend component module.
...
Cover these exact outcomes: 11 selected files are rejected with a visible alert; a file of `10 * 1024 * 1024 + 1` bytes is rejected; a rejected `uploadFiles` promise retains both the selected attachment and note; a resolved promise clears them.
...
Before merging selected files, reject sanitized-name duplicates, totals over 10, and files over 10 MiB through a new optional `onAttachmentError(message: string)` callback wired to `setDraftError`. Do not remove already valid pending files when a later selection is rejected.

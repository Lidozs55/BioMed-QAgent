# 归档：V2 Pipeline Refactor 开发 TODO（已全部完成）

> **归档说明（2026-08-13）**：本文件是旧主线「V2 Pipeline Refactor」
> （[../BioMed-QAgent_Pipeline_Refactor_Design.md](../BioMed-QAgent_Pipeline_Refactor_Design.md)
> §16 Phase 1-8）的完整历史清单，除两项 P2 维护项（MAX_TURNS 指导、设置页列表分页）
> 外全部条目已完成并经全量回归（含 Phase 8 清理与设置页模型配置重构）；该两项已
> 转入 [../TODO.md](../TODO.md) 的「独立维护项」。开发主线已切换为 Pi 迁移方案，本
> 文件不再维护，仅供查阅历史。
> 文内相对链接原指 `docs/` 目录（如 `docs/REVIEW_*.md`）；归档后请按

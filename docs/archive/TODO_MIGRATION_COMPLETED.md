# 归档：Pi 迁移主线开发 TODO（已完成部分）

> **归档说明（2026-08-17）**：本文件是 [../TODO.md](../TODO.md) 已完成部分的
> 完整历史清单——迁移主线（Phase 0-9）、M2 集成收口、独立维护项完成部分、
> 前端 UI 合规修复与已登记 issue 的已解决条目（含 git 卫生收口）。细节与
> 证据另见 [../migration/](../migration/)（迁移执行记录）、[../adr/](../adr/)
> （ADR-017 起）与 git 历史，本文件不重复解释。当前清单只保留开放条目与
> 当前约束（见 [../TODO.md](../TODO.md)），本文件不再维护，仅供查阅历史。
> 文内相对链接原指 `docs/` 目录（如 `docs/migration/*`）；归档后请按
> `docs/archive/` → `docs/` 一层回退理解。

> 迁移执行期间的历史见证（2026-08-17 起新增内容请勿回填到此文件）。
...
### 长上下文 `stopReason="length"` 截断不再假完成（✅ 完成，2026-08-17，TASK-046）
...
### git 卫生（2026-08-17，[x] 已完成 2026-08-17）

- [x] **P2** .gitignore 已新增 `_tmp*`、`.tmp-*` 规则。`docs/runs-log.md`
      （运行记录文档）与 `server/tests/phase5/fixtures/geo/gpl10332_soft.txt.gz`
      （geo-probe-mapping.test.ts 依赖的 fixture）已确认被 git 跟踪，不会按
      tmp 清理。
- [x] **P2** `docs/runs-log.md` 已补记早期 e2e-gold2-* run 的编码损坏红旗
      （2026-08-17「UTF-8 编码损坏红线（gold2 提交侧）」小节，含两类损坏形态
      与分层拦截约定）。

...
17. **Core-owned product closure**：动态 `assessment_policy_ref` 必须命中 Core 注册的产品拓扑清单；prepare receipt 绑定清单 digest，submit 时重算。候选在 B3/HIL 前必须精确闭合清单要求的 family、table ID/role/schema、relation ID 与最小行数。Agent 自报清单、未知 policy 或缩减后的单表 projection 均 fail closed，不能进入 `publication_acceptance`。

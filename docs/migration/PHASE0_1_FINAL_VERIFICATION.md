...
The final `main` verification also recreated the root pnpm Workspace from the frozen
lockfile. That clean install exposed a missing `@biomed/contracts` build prerequisite;
the server now declares the prerequisite in `predev` and `pretest`, and the root test gate
passed from the rebuilt environment without relying on stale `dist` output.

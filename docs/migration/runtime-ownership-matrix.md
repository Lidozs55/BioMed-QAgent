...
| `SkillCatalog` and `UserSkillStore` | FastAPI lifespan | Legacy FastAPI lifespan; minimal Pi Skill root owned by Pi adapter | Pi Skills plus a TS management boundary | Do not migrate the full catalog | Legacy lifespan; Pi adapter loads only Phase 1 smoke and dataset-construction Skills | Process/session disposal; no independent legacy close today | Bad legacy user packages remain unavailable rather than aborting the process; missing experimental Skill fails the call without leaking the session |

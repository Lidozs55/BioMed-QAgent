...
> - **运维新陷阱（非卡点）**：`scripts/build-contracts-if-needed.mjs` 在 pnpm 硬链接式 workspace 安装下 `syncInstalledContracts` 对 `node_modules/@biomed/contracts`（非 junction 的硬链接副本）cpSync 自拷贝报 "src and dest cannot be the same"，`pnpm dev`/`pretest`/contracts `prebuild` 全挂。**→ 已修（2026-09-02，fix/gold-gate-relaxations）：同文件判定改按 dev+ino 身份，硬链接克隆跳过拷贝。** ⚠️ H3 变体再实证（09-02 合并 origin 批次）：合并涉及 `packages/contracts/src` 的提交后，server vitest 会解析 node_modules 的**旧 dist**（合并 origin 的 `binding_kind` 新闸后 17 个 dynamic-family 用例假红），`pnpm build` 重建 dist 后全绿——**合并 contracts 源码变更后必须先 build 再跑 server 测试**。另：origin `ad4bc65e` 给 `writeJsonAtomic` 加的目录 fsync 在 Windows 抛 EPERM（本机全部持久化写入红测），已修（`8ad94e0f`，win32 跳过目录 fsync）；`semantic-route-fence` 的 symlink 用例需符号链接特权，登记 ISSUES。
>

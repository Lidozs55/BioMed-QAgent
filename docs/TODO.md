...
- [ ] **Gold6 live 图表 evidence 到正式 Publication 证据归档。** 代码级 fixture 闭环已存在；R4 已 live 证明显式 `provenance_only` 能让真实 binary archive member 进入 formal dependency closure 而不进入 UTF-8 Transform Host，R3 的 supplementary member admission 死锁关闭。但正式 Publication 仍未出现：生产 `registered_paper_chart_evidence` v1.2.0 的 carrier/derived provenance 不写 semantic validator 要求的 `evidence.manifest.{charts,points}`，导致 committed candidate 被 typed semantic rejection；另有 86 series / 0 points。**原“补非空 VLM point + point review closure”路线已被 ADR-043 取代：Gold6 应走 exact source-data；无精确点则审计弃权并保留独立精确表格值。**先以真实 producer 输出增加失败回归并统一 bytes-bound manifest，再按 exact-only 迁移验收；修复前不启动下一轮。验收仍是同一 Host/commit 下 `publication_acceptance`、B3、OperationResult、Manifest 与 Artifact API SHA-256 全链；`ua_*` 与 workspace 文件不得替代正式产物。

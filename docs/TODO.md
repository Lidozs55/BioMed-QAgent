...
- [ ] **M8 VLM 通道量化与配置依据（2026-09-06 更新）。** 已落地：提取范围声明（只转录图内明确数值标注、禁止对无标注图声称精确读数、原始数据检索路径、至多文字趋势简报，报告 4.2.3）、"降级失败"显式定义、置信度三档未校准声明、medium 记录发布路径披露。剩余缺口：(a) 校准/误差率实验——随 gold6 类重跑执行，对图内明确标注值做人工核对参考，统计转录错误率与 gate 拒绝率（范围声明使需要标注的对象大幅减少）；(b) 可选低成本的档位实测：同批图 qwen3.8-flash 与 max 各提取一次比对一致率，补档位选择的实验依据（当前仅有"任务受限转录+门禁兜底"的结构性理由）；(c) 若评委追问趋势简报的形态，在演示或附录展示一次文字趋势简报输出。

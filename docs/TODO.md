...
- [ ] **冻结单 Host Gold1–Gold6 证据。** 在同一 commit、同一 Host 与同一 data root 上记录 task/run/requirement、registered input、OperationResult、B3、ProductAssessment、Publication 与 Artifact API hash 证据；缺失证据必须标为 blocked/unknown，不能用历史产物补齐。
  ...
  - 进展（2026-08-31，`fix/gold6-r3-closure`）：上述五项代码修复已集成：governed carrier 走 exact-byte committed derived provenance；单页 schema 错误和 clear-series 无点各有一次有界纠错重试，失败页隔离且无点 series 降为 unclear；Gold assert 游标分页；supervisor adopt/resume exact durable reconciliation；Gold6 E2E 的 XML/PDF/ZIP 全部走 acquisition-only surface。定向回归 41/41 通过。剩余是完整质量门与唯一 pure-topic R3 live closure；R1/R2 都不能补记为通过。

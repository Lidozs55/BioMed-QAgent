...
- [ ] **冻结单 Host Gold1–Gold6 证据。** 在同一 commit、同一 Host 与同一 data root 上记录 task/run/requirement、registered input、OperationResult、B3、ProductAssessment、Publication 与 Artifact API hash 证据；缺失证据必须标为 blocked/unknown，不能用历史产物补齐。
  ...
  - 进展（2026-09-01，唯一 R4 `42984ecb`）：用户明确跳过完整 workspace 门禁后，以既有 focused 5 files / 48 tests + test TypeScript/改动文件 ESLint 证据直接复测。task `task_ts_b41c…991a` / run `run_ts_7d686…784f` 自然 completed，但仍为 `blocked_no_publication`（10,478 events，SHA-256 `97073ddd…b0ef`，0 Publication/Artifact/acceptance HIL）。R3 的 P5 已 live 穿透：preflight 是 3 个 JSON `transform_input` + 10/13 个 `provenance_only`，真实 JPEG member `asset_38428f…e9c4` 不进 Host 且成功参与 VLM carrier `asset_4de014…90d4` 的父闭包。新 blocker 收窄为 producer/validator 契约错位：VLM derived provenance 不写 `evidence.manifest`，semantic validator 却强制读取它；5 次 typed 拒绝的 30 个 `ua_*` exact bytes 均重验通过且保持 untrusted，不计正式产物。三篇仍为 86 series / 0 points。证据已归档至 `data/gold-runs/42984ecb1c43-gold6-qwen38flash-r4-standard/`；R4 不得补记为通过。
- [ ] **端到端图表 Gold 案例（对齐加分项"自动识别图表坐标轴或图例解析错误"）。** 历史兼容的图表 evidence 闭环已存在（见下方历史兼容项），但 exact-only source-data 正向路径和 successor case 尚未完成；gold7–9 也未有含图表场景的端到端案例。评审明确将其列为最高优先级失分点（"图表数据处理"是赛题核心能力）。补一个自然语言 TOPIC 出发、含论文图表抽取与坐标轴/图例核验的端到端 Gold 案例。**原“低置信度图像点经 HIL 修正后发布”验收已被 ADR-043 取代。**
  - 验收：TOPIC → VLM/PDF/caption 定位 chart series → 显式 numeric source-data acquisition/parser → chart 四表（chart_series / chart_points / papers / sources）→ 正式 Publication；坐标轴名/单位/刻度与图例状态经 `axis_validation_status` / `legend_validation_status` 门禁；非空 points 必须逐行闭合到 exact numeric source；找不到精确点时允许审计后的空表并在最终报告披露；单位 token 不漂移；评审可经报告 5.5 所述 API/前端复现。
  ...
  - 进展（2026-08-30，`feat/gold6-t8-closure`，Task 8 steps 1–4，历史兼容 fixture）：Gold6 侧 runner 已把冻结约束送入 Run（T1），prompt/skill 已教学"冻结 execution_context 绑定任务语义但绝非发布权威、carrier/VLM/locator/review 不可用即结构化 blocker"；旧 e2e 曾验证低置信度点经 HIL correct 后 original 值保留并进入正式 Publication。该 estimated-point 验收路径已由 ADR-043 取代，不代表当前 exact-only 能力；当前应改走 exact source-data 或审计弃权。
...
- [x] **Canonical 图表 evidence 到正式 Publication 闭环（历史兼容代码能力；ADR-043 后不作为当前 exact-only 能力）。** 将现有 `bioactivity-measurement/chart-evidence` 模块接入受控的 Family Registry、Adapter/Assembler、Validation、ProductAssessment 与 Publisher 路线；VLM/PDF/caption 输出必须先成为 task-owned、摘要绑定的 evidence asset，不能让任意 workspace CSV 直接获得正式发布权。
  - 验收（2026-08-29，历史兼容闭环）：chart 四表进入生产 `bioactivity_measurement` family（schema + registered JSON parsers + 组装分派），点级 provenance/review 门在组装前 fail-closed（结构化 `chart_evidence:chart_evidence_gate` 检查写入 validation_report，不产生 Publication），经 B3 + Publisher 走既有原子发布。旧点级 Gold（`server/tests/chart-evidence-publication-closure.test.ts`）覆盖 accepted/corrected（HIL correction 保留 original 值与 `human_correction` 步骤）、artifact bytes 与 SHA-256 重算、pending review 与缺表拒绝；`publication_created`/`artifact_produced` 事件重放由既有 durable-runtime 测试锁定。该 estimated-point/review 语义已被 ADR-043 supersede，不能作为当前 exact-only 验收；当前正向验收见 P1 全局迁移项。
...
- [x] **百炼联网搜索 `search_info` 透出（模型侧内置搜索的可见性）。** 设置页「联网搜索」（`enable_search`）开启后，百炼兼容端点在响应中返回 `search_info`（命中的网页结果/来源）。经 fetch 层流镜像捕获 → `provider_search_info` durable 事件 → 消息聚合 → 前端折叠「联网搜索来源（N）」四层实现，与"通用 Web 搜索工具"（Agent 侧受控搜索）互补，不替代取证链（检索结论仍不得直读为正式证据）。完成（2026-09-03，`fix/search-info-projection`）；设计决策与验证见 [handoff](handoffs/2026-09-03-bailian-web-search-audit-and-fixes.md) §五。
  - 验收：开启联网搜索的会话能在前端看到每次命中百炼侧检索的结构化摘要 ✅；关闭时零开销（不注册探针/零渲染）✅；不破坏 authoritative 事件 schema 兼容性（旧 events.jsonl 重放不受影响）✅。遗留：百炼 Key 的 live 端到端冒烟待首次配 Key 后执行。
...
- [x] **硬编码参数审计整改。** 2026-09-02 完成 [`audit/2026-09-02-hardcoded-params-audit.md`](audit/2026-09-02-hardcoded-params-audit.md) 批准范围：P0 双源真相接线；新增模型请求/恢复、Core acquisition、JSON 响应与 VLM PDF 预算；视觉 Temperature 复用 managed model 参数并把 governed 参数纳入证据摘要；浏览器并发与事件缓存进入 Host env；上传安全上限收敛为 contracts 单源。Publication/Validation、协议、parity、像素与溯源常量保持不可配置。运行语义与 Host 清单见 [`architecture/runtime-limits.md`](architecture/runtime-limits.md) 和 [`DEVELOPER_QUICKSTART.md`](DEVELOPER_QUICKSTART.md)。
  - 验收：新增字段保存/越界/重置、Run 快照、Pi/VLM retry、PDF 参数溯源、Core acquisition 退避、六工具响应收紧、Host env 解析和上传契约均有定向测试；workspace 质量门结果在整改提交中留证。
...
## 论文整改遗留（2026-09-05 评审 M1–M9 核验后，文字回应已落地，以下为需运行数据/实验支撑的硬项）

2026-09-05 对本地 LaTeX 报告的九点评审（M1–M9）经逐条核验基本成立；表述层面的问题（效率结论归一、覆盖口径、可复现边界、信任边界措辞、HIL 验证范围、可靠性表列名、VLM 机制边界、对照口径披露）已在 `docs/latex/` 同日修改。以下硬项需要新证据，暂无法用文字解决：
...
- [ ] **M7 checkpoint/replay 运行级证据。** 设计一次中断–检查点–重放演示，取得独立事件证明后更新表 4.2 该行与报告 6.5（本版已如实声明无证据）。相关：P1"checkpoint/reuse/restart 闭环"。

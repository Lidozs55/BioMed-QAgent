...
- [ ] **M1 数据值正确性对照评测。** 主体即 P0"参考集对照准确性评测（G1 evaluator）"条目；补充三点：样例1 两环境互查需补抽样协议与共有记录数并更新报告 5.4.2；样例6 图表通道统计提取点数、误差分布与 `chart_evidence_gate` 排除量并写入 5.4.3（本版已将量化归属参考集对照评测）。
- [x] **M3 完备性疑点核实（2026-09-06 完成）。** 根因确认为 ChEMBL 采集提供者单请求硬编码 limit=100 且无翻页（`chembl-provider.ts`，代码级证据），结论已写入报告 5.2.1（定性为采集缺陷，注明缓解与重跑口径）；样例4 文献 2 行的中性说明同步写入。分页修复与上限可配置化已合入 dev，完整翻页立项见 P1。
...
- [ ] **M8 VLM 通道量化与配置依据（2026-09-06 更新）。** 已落地：提取范围声明（只转录图内明确数值标注、禁止对无标注图声称精确读数、原始数据检索路径、至多文字趋势简报，报告 4.2.3）、"降级失败"显式定义、置信度分级语义声明（确定性分级驱动门禁，统计误差率属参考集对照评测）、medium 记录发布路径披露。剩余缺口：(a) 校准/误差率实验——随 gold6 类重跑执行，对图内明确标注值做人工核对参考，统计转录错误率与 gate 拒绝率；(b) 可选：同批图 qwen3.8-flash 与 max 转录一致率对照；(c) 趋势简报的演示/附录展示形态。

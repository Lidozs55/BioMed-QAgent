...
- [ ] **Gold6 live 图表 evidence 到正式 Publication 证据归档。** 代码级 fixture 闭环已存在；R4 已 live 证明显式 `provenance_only` 能让真实 binary archive member 进入 formal dependency closure 而不进入 UTF-8 Transform Host，R3 的 supplementary member admission 死锁关闭。但正式 Publication 仍未出现：生产 `registered_paper_chart_evidence` v1.2.0 的 carrier/derived provenance 不写 semantic validator 要求的 `evidence.manifest.{charts,points}`，导致 committed candidate 被 typed semantic rejection；另有 86 series / 0 points、无 point review IDs。先以真实 producer 输出增加失败回归，统一 producer/validator 的 bytes-bound manifest 契约，再补非空 point evidence 与 review closure；修复前不启动 R5。验收仍是同一 Host/commit 下真实 `vlm_extraction` 数据审查、`publication_acceptance`、B3、OperationResult、Manifest 与 Artifact API SHA-256 全链；`ua_*` 与 workspace 文件不得替代正式产物。

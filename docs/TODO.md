...
- [ ] **百炼联网搜索 `search_info` 透出（模型侧内置搜索的可见性）。** 设置页「联网搜索」（`enable_search`）开启后，百炼兼容端点在响应中返回 `search_info`（命中的网页结果/来源），当前 pi 适配链路将其忽略：模型内部消费检索结论，但 UI 与证据链看不到"模型查过什么网"。在事件投影/消息元数据层透出 `search_info` 摘要（来源 URL/标题有界列表），与上方"通用 Web 搜索工具"（Agent 侧受控搜索）互补，不替代取证链（检索结论仍不得直读为正式证据）。背景见 [handoff](handoffs/2026-09-03-bailian-web-search-audit-and-fixes.md) §三。
  - 验收：开启联网搜索的会话能在前端看到每次命中百炼侧检索的结构化摘要；关闭时零开销；不破坏 authoritative 事件 schema 兼容性（旧 events.jsonl 重放不受影响）。

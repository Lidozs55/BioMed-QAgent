"""公共基类与工具：DataRecord 加载/保存、字段映射构造、CLI 设置、统计。

被 field_aligner.py / unit_normalizer.py / duplicate_dedector.py 复用。
...
def setup_cli(name, description):
    """返回 argparse.ArgumentParser，预设通用参数 --input/--out/--dictionaries/--task-id。"""
    parser = argparse.ArgumentParser(prog=name, description=description)
    parser.add_argument("--input", required=True, help="输入 DataRecord JSON 文件或目录")
    parser.add_argument("--out", required=True, help="输出 JSON 文件路径")
    parser.add_argument("--dictionaries", default="dictionaries/",
                        help="同义词字典目录路径（可选，缺失时使用内置默认字典）")

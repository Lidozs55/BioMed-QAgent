"""Hub 基因识别 + 上游调控因子反查（基于 PPI 结果 / STRING）。

输入：
  --input：PPI 分析结果 JSON（含 stats_table 与 chart_data），或 DataRecord JSON
  --ppi-result：可选，已有 PPI AnalysisResult JSON 路径；为空则从 --input 提取基因后内部调用 STRING
功能：
  1. 从 PPI 结果或 --input 提取 gene 列表与 degree/betweenness/closeness
...
用法：
    python scripts/analysis/hub_gene_analyzer.py \
        --input ppi.json --out hub.json --task-id t1
    python scripts/analysis/hub_gene_analyzer.py \
        --input cleaned.json --ppi-result ppi.json --out hub.json
"""
...
def _fetch_string_network(genes, species, score_threshold):
    """调用 STRING network 端点获取互作边（--ppi-result 缺失时使用）。"""
    import requests
...
def _build_centralities(genes, species, score_threshold):
    ...
    try:
        ...
        log_stderr(f"networkx 不可用，降级为 degree 直算: {e}")
...
def run_hub_gene_analysis(genes, degrees, betweenness_map, closeness_map,
    ...
    for gene in sorted(hub_set):
        ...
        try:
            ...
            log_stderr(f"{gene} STRING 互作伙伴 {len(partners)} 个")
            for p in partners:
            ...
            log_stderr(f"{gene} 上游 TF 反查失败，降级: {e}")
            tf_available = False
...
def main():
    parser = setup_cli("hub_gene_analyzer", "hub 基因识别 + 上游调控因子反查")
    parser.add_argument("--ppi-result", default=None,
                        help="已有 PPI AnalysisResult JSON 路径；为空则从 --input 提取基因后内部调用 STRING")
    ...
    try:
        if args.ppi_result:
            genes, degrees, betw, close, chart_data, modules = \
                _resolve_from_ppi_file(args.ppi_result)
            log_stderr(f"从 --ppi-result 加载 {len(degrees)} 节点")
        elif args.input:
            ...
            if isinstance(obj, dict) and "result" in obj and isinstance(obj["result"], dict):
                obj = obj["result"]
            stats_table = obj.get("stats_table", []) if isinstance(obj, dict) else []
            if stats_table and any(isinstance(r, dict) and "degree" in r
                                   for r in stats_table):
                # --input 本身是 PPI 结果
                genes, degrees, betw, close, chart_data, modules = \
                    _resolve_from_ppi_file(args.input)
                log_stderr(f"从 --input（PPI 结果）加载 {len(degrees)} 节点")
            else:
                # --input 是 DataRecord，提取基因后内部调用 STRING
                genes = extract_genes(records)
                log_stderr(f"从 --input 提取基因 {len(genes)} 个，调用 STRING 构建 PPI")
                built = _build_centralities(genes, args.species,
                                            args.score_threshold)
                if built is None:
                    degrees, betw, close = {}, {}, {}
                    chart_data, modules = {"nodes": [], "edges": []}, 0
                else:
                    degrees, betw, close, chart_data, modules, _ = built
        else:
            save_error("hub 基因分析需要 --ppi-result 或 --input 参数", args.out)
            sys.exit(1)

        result = run_hub_gene_analysis(genes, degrees, betw, close, chart_data,
                                       modules, args.species, args.score_threshold,
                                       args.task_id)
        save_result(result, args.out)
        log_stderr(f"hub 基因结果已写入 {args.out}")
    except Exception as e:
        save_error(f"hub 基因分析失败: {e}", args.out)
        sys.exit(1)

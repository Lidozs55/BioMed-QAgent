...
输入：
    --input network.tsv --format (auto|string|sif|graphml，默认 auto)
...
示例：
    python network_parser.py --input network.tsv --format auto --out result.json
...
def main():
    parser = setup_cli("network_parser", "网络文件解析器")
    parser.add_argument("--format", default="auto",
                        choices=["auto", "string", "sif", "graphml"],
                        help="网络格式（默认 auto 自动检测）")
    args = parser.parse_args()
    try:
        p = NetworkParser()
        records = p.parse(args.input, fmt=args.format)
        for r in records:
            r["task_id"] = args.task_id
        write_output(make_ok(records), args.out)
    except Exception as e:
        write_output(make_error(f"网络文件解析失败: {e}"), args.out)
        sys.exit(1)

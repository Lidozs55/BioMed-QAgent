...
Verifies:
- register_all_skills() registers all 69 entries (67 unique after dedup)
- 46 active, 21 dormant
...
def test_registry_total_count():
    """All 69 entries from _TOOLS_METADATA should be registered as skills."""
    count = register_all_skills()
    assert count == 69, f"Expected 69 entries processed, got {count}"
    assert get_skill_registry().count() == 67


def test_active_vs_dormant_count():
    """46 active + 21 dormant = 67 unique."""
    ...
    assert len(dormant) == 21, f"Expected 21 dormant, got {len(dormant)}"
...
def test_category_counts():
    ...
    registry = get_skill_registry()
    expected = {
        "datasources":  36,  # 15 active + 21 dormant
        "parsers":       6,
        "cleaners":      3,
        "analysis":      7,
        "io":            4,

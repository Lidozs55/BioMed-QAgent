"""Markdown 报告生成工具（BioMed QAgent Stage 6）。
...
模块导入示例：
    from .export.to_report import build_report, render_markdown
    sections = build_report(records, lineage, task_id, analysis_dir, out_path)
    md = render_markdown(sections)

复用：to_docx 通过 import build_report 复用本脚本的内容生成逻辑。
block 类型：para{text} / bullets{items} / table{headers,rows}。
"""
import json
from pathlib import Path

from ._base import load_lineage, load_records, statistics, utc_now

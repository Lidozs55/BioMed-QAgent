...
模块导入示例：
    ...
    data = load_json("results/diff_expr.json")
    plot_volcano(data, output_path="charts/volcano.png", title="Volcano Plot")
"""
from ._base import (
    DOWN_COLOR,
    NS_COLOR,
...
def plot_volcano(data, output_path, title=""):
    """绘制火山图。

    Args:
        data: AnalysisResult 解析后的 dict（含 gene/log2fc/adj_p_value）
        output_path: 输出 PNG 图片路径
        title: 图表标题（可选，空串则用默认 "Volcano Plot"）

    Returns:
        dict: {"chart": output_path, "data_points": N}

    Raises:
        ValueError: 输入未找到基因数据，或无有效数据点
    """
    genes = _extract_genes(data)
    if not genes:
        raise ValueError("输入中未找到基因数据（需含 log2fc / adj_p_value 字段）")
    ...
    if not xs:
        raise ValueError("无有效数据点（log2fc 或 adj_p_value 缺失/非数值）")
    ...
    return {"chart": output_path, "data_points": len(xs)}

...
模块导入示例：
    ...
    result = run_survival_analysis("TP53", "TCGA-PAAD", None, 200, "task-1")
"""
...
def _fetch_gdc_expression(gene, cohort, samples, max_samples):
    ...
    建议通过 input_path 提供预先准备好的表达矩阵以获得真实结果。
    """
    import requests
    try:
        ...
        if not hits:
            logger.warning(f"GDC 未找到基因 {gene}，表达数据降级")
            return None
        # GDC 样本级表达需下载 HTSeq 文件（每样本一个，过重），此处降级
        logger.warning("GDC 表达获取需下载 HTSeq 文件，跳过（建议用 input_path 提供表达矩阵）")
        return None
    except Exception as e:
        logger.warning(f"GDC 表达查询失败，降级: {e}")
        return None
...
def _survival_stats(time_high, event_high, time_low, event_low):
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Cox 拟合失败，HR 用 O/E 比: {e}")
        return p, hr, ci_low, ci_high
    except Exception as e:
        logger.warning(f"lifelines 不可用，降级纯 Python KM + log-rank: {e}")
...
def _insufficient_result(gene, cohort, task_id, reason):
    ...
    return {
        ...
        "summary": f"{gene}（{cohort}）生存分析数据不足：{reason}；"
                   f"可通过 input_path 提供 TCGA 表达+临床数据",
        "created_at": utc_now(),
    }
...
def run_survival_analysis(gene, cohort, input_path, max_samples, task_id):
    ...
    if input_path:
        ...
        logger.warning(f"从 input_path 加载表达 {len(expression) if expression else 0} 样本，"
                       f"临床 {len(clinical) if clinical else 0} 条")
    else:
        try:
            clinical = _fetch_gdc_clinical(cohort, max_samples)
            logger.warning(f"GDC {cohort} 临床数据 {len(clinical)} 条")
        except Exception as e:
            logger.warning(f"GDC 临床数据获取失败: {e}")
            clinical = []
    ...
    if not expression or not clinical:
        reason = ("表达数据缺失（GDC 不提供轻量 per-gene 表达端点）"
                  if not input_path else "input_path 中表达/临床数据缺失")

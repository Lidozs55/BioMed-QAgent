...
作为模块使用：
    from app.tools.datasources.openalex import search_openalex
    records = search_openalex("pancreatic cancer", 20, "task-1")
"""
...
def search_openalex(query: str, max_results: int, task_id: str) -> list[dict]:
    ...
    for attempt in range(5):
        ...
        if r.status_code == 429:
            ...
            logger.warning("openalex: 429 限流，%ss 后重试（第 %s/5 次）", wait, attempt + 1)
            _time.sleep(wait)
            continue
        break
    if r is None:

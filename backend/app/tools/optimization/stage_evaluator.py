...
阈值：见 ._base.DEFAULT_THRESHOLDS，可由 thresholds_override 覆盖

模块导入示例：
    from .stage_evaluator import evaluate
    from ._base import load_records, save_evaluation

    records = load_records("cleaned.json")
    evaluation = evaluate(records, stage="clean", iteration=1,
                          task_id="T1", expected_entities={"gene": {"TP53"}})
    save_evaluation(evaluation, "eval_clean.json")
"""
from __future__ import annotations

from ._base import DEFAULT_THRESHOLDS, make_evaluation

...
模块导入示例：
    from .keyword_expander import expand_keywords, _build_alias_index, _parse_entities_str
    from ._base import load_records

    records = load_records("cleaned.json")
    expected = _parse_entities_str("TP53,quercetin,pancreatic_cancer")
    alias_index = _build_alias_index("dictionaries/")
    queries, new_entities, by_strategy = expand_keywords(records, expected, alias_index)
"""
...
def _parse_entities_str(entities_str):
    ...
    输入格式：逗号分隔的实体列表。支持 'gene:TP53,compound:quercetin' 显式标注，
...
def expand_keywords(records, expected_entities, alias_index):
    ...
    Args:
        records: 已有的 DataRecord 列表
        expected_entities: 用户查询中的期望实体集合，按类型分组
            {"gene": {"TP53"}, "compound": {"quercetin"}, ...}
        alias_index: 由 build_alias_index 构造的同义词反查索引

    Returns:
        tuple: (新查询串列表, 新实体列表, 按策略分组字典)
            - 新查询串列表: list[str]
            - 新实体列表: list[{"type":..., "value":...}]
            - 按策略分组: {"synonym":[...], "missing_entity":[...], "cross_entity":[...]}
    """

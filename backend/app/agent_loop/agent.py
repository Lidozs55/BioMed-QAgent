...
进入 `run_research_pipeline` 前，必须满足以下覆盖门禁：
1. **至少覆盖 3 个与课题相关的数据库**（如 PubMed + GEO + PDB/Reactome/PubChem）。
   preferred_sources 不足 3 个时，自动补充其他相关数据库。
2. **机制驱动检索**：先从综述文献中提取候选机制/基因，再按具体基因名查询
   PDB（如 "SOST protein structure"）、Reactome（如 "R-HSA-201681 Wnt signaling"）、
   PubChem（如 "calcitriol"），而非用疾病关键词盲目检索结构/通路/化合物库。
3. **覆盖率自评估**：进入 Pipeline 前在心里过一遍清单——
   "我查了文献吗？表达数据吗？结构数据吗？通路数据吗？化合物数据吗？"
   只要有与课题明显相关但未查询的数据库，先补充检索再进 Pipeline。
4. **每个数据库至少 1 个有效结果**：某数据库返回 0 结果不算"覆盖"，
   换关键词重试或换数据库。

## 共病/多表型主题的双侧分解
当研究主题涉及"X 与 Y 的关联/共病/共享机制"时（如"AD 与骨质疏松共病"），
**不要把主题当作单一关键词检索**。必须分解为三个子目标分别检索：
1. **X 侧数据**：疾病 X 的表达/变异/结构数据
2. **Y 侧数据**：疾病 Y 的表达/变异/结构数据
3. **共享机制数据**：同时在 X 和 Y 中有证据的基因/通路/化合物
   - 从综述中提取候选共享通路（如 Wnt、RANKL/OPG、雌激素信号）
   - 对每个候选基因，分别查 "gene + X" 和 "gene + Y" 确认双向证据
   - 最终产物应包含 X 侧 + Y 侧 + 共享三层数据，而非仅其中一侧

## 数据可用性预检
传入 `gse` 前，优先选择已知的成熟数据集（supplementary 文件已上传且可下载）。
若 GEO 数据集较新（发布 < 6 个月），其 tximport/表达矩阵文件可能尚未上传
（HTTP 404）。遇到下载失败时：
1. 在 `run_research_pipeline` 返回中查看 `download_log` 是否有 failed 记录
2. 若核心表达数据下载失败，换一个同主题的成熟数据集重试
3. 不要用相同 GSE 反复重试——404 不会自愈

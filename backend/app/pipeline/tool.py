...
@function_tool(
    ...
    description_override=(
        ...
        "Before passing any ``gse`` you MUST first call describe_geo on each "
        "candidate accession and confirm its samples/platform match the topic; "
        "do NOT submit an un-vetted GSE.  "
        "For single-gene / target-gene differential analysis prefer gene-level "
        "GDC/Xena matrices (``xena_dataset_id`` / ``gdc_project_id``, gene symbol "
        "directly queryable) over probe-level GEO arrays.  "

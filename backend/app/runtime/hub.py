...
from app.domain.contracts import AssistantStreamFrame, EventEnvelope
...
class AssistantStreamSubscription:
    """One connection's filters and bounded ephemeral frame queue."""

    def __init__(
        self,
        hub: AssistantStreamHub,
        *,
        task_ids: Iterable[str],
        queue_size: int,
    ) -> None:
        self._hub = hub
        self._task_ids = set(task_ids)
        self._queue: asyncio.Queue[AssistantStreamFrame] = asyncio.Queue(
            maxsize=queue_size
        )
        self._closed = False
        self._overflowed = False
    ...
    async def receive(self) -> AssistantStreamFrame:
        if not self._queue.empty():
            return self._queue.get_nowait()
        if self._overflowed:
            raise SubscriberOverflowError("subscriber event queue overflowed")
        if self._closed:
            raise SubscriptionClosedError("subscription is closed")
        queued_frame = asyncio.create_task(self._queue.get())
        closed = asyncio.create_task(self._closed_event.wait())
        try:
            done, _ = await asyncio.wait(
                {queued_frame, closed},
            ...
            pending = [task for task in (queued_frame, closed) if not task.done()]
            for task in pending:
                task.cancel()
            await asyncio.gather(*pending, return_exceptions=True)
        if queued_frame in done:
            return queued_frame.result()
        if self._overflowed:
            raise SubscriberOverflowError("subscriber event queue overflowed")
        raise SubscriptionClosedError("subscription is closed")
    ...
    def _matches(self, frame: AssistantStreamFrame) -> bool:
        return frame.task_id in self._task_ids
...
class AssistantStreamHub:
    """Fan out ephemeral assistant frames without blocking their producer."""
    ...
    def __init__(
        ...
        *,
        ...
        if subscriber_queue_size < 1:
            raise ValueError("subscriber_queue_size must be positive")
        self.subscriber_queue_size = subscriber_queue_size
        self._subscribers: set[AssistantStreamSubscription] = set()
        self._lock = asyncio.Lock()
    ...
    async def publish(self, frame: AssistantStreamFrame) -> None:
        async with self._lock:
            overflowed: list[AssistantStreamSubscription] = []
            for subscription in self._subscribers:
                if not subscription._matches(frame):
                    continue
                try:

...
借鉴临床流行病学报告标准（STROBE / TRIPOD(+AI) / REMARK / STARD(+AI) /
PROBAST(+AI) / SPIRIT-AI / CONSORT-AI 等）的核心主张，按产物类型选用对应的
披露口径，而不是套用某一篇报告模板：

- **选择报告标准**：按产物与数据分析类型对齐 —— 观察性队列 → STROBE；预测/风险
  模型 → TRIPOD；标志物/预后 → REMARK；诊断准确性 → STARD；风险偏差评估 → PROBAST。
  BioMed QAgent 面向**研究与评估**场景，不用于个体患者诊疗判断；所有结论按
  aggregate-only 层面陈述，不宣称对个体做出诊断或治疗决定。
- **必填产物抬头**：为每个正式产物带上出处与适用范围 —— 来源队列/样本量/纳入排除、
  结局与暴露定义、分析层面（研究级 vs 个体级）、数据局限与缺失处理、生成 pipeline 与
  版本。缺失这些元信息的结论视为不可复现。
- **证据分级式陈述局限**：描述不确定性时区分 偏倚风险（入选/测量/缺失）、间接性
  （替代结局/跨队列外推）、不精确性（样本量/置信区间），并用 GRADE 式语言标明
  “证据强度”，避免用研究数据断言确定性因果。
- **结论与数据分层**：把“观测到的事实”（溯源可闭、字段可查）与“解释/推断”（基于
  方法假设）分开陈述，明确哪些是数据直接支持、哪些是方法学判断。
- **去标识与合规**：涉及个体级数据时遵循最小化原则，检查是否含个人信息与再识别
  风险（如可回推患者/受试者的 ID、日期、稀有组合）；来源许可与用途边界一并记录。

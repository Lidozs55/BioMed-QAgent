...
- For a formal bioactivity measurement compound-identity requirement, use one verified positive CID; the fixed acquisition accepts one CID per binding, so use one binding/request per compound CID. On the static route use a binding with `source="pubchem"`, `adapter_id="bioactivity.pubchem_identity.v1"`, and builtin `provider_id="pubchem.files.v1"`; omit source files. In `submit_dynamic_family_publication`, put the equivalent fixed provider and parameters under the acquisition requests field. Dataset Core must refetch, register, and provenance-bind the immutable PubChem JSON response. Do not inspect the registered file through workspace tools.

...
import {
  ...
  resolveDatasetExecutionProposal2,
  type ExecutionSpecResolutionContext,
} from "../src/dataset/execution-spec-readmission/index.js";

const A = "a".repeat(64);
const B = "b".repeat(64);
const C = "c".repeat(64);
const D = "d".repeat(64);
const TASK_ID = "task_execution_spec";
const REQUIREMENT_ID = "requirement_execution_spec";
const GENERATION = 7;
...
async function fixture(overrides: {
  assets?: ExecutionSpecResolutionContext["assets"];
  results?: ExecutionSpecResolutionContext["results"];
  transforms?: ExecutionSpecResolutionContext["transforms"];
  policies?: ExecutionSpecResolutionContext["policies"];
  family?: Partial<FamilySpec>;
} = {}): Promise<Fixture> {
  const family = await familySpec(overrides.family);
  const assetId = `asset_${A}`;
  const proposal: DatasetExecutionProposal2 = {
    schema_version: "2.0",
    spec_kind: "proposal",
  ...
  const context: ExecutionSpecResolutionContext = {
    task_id: TASK_ID,
    requirement_id: REQUIREMENT_ID,
...
): Promise<ExecutionSpecResolutionError> {
  try {
    await operation();
  } catch (error) {
    if (error instanceof ExecutionSpecResolutionError) return error;
    throw error;
  }
  throw new Error("expected ExecutionSpecResolutionError");
}

describe("dataset execution spec Core readmission", () => {
  test("resolves exact registered asset/result bindings and emits wire evidence", async () => {
    const { proposal, context } = await fixture();
    const result = await resolveDatasetExecutionProposal2(proposal, context);
  ...
  test.each([
    ...
    ["stale generation", { assets: [{
      ...
      task_id: TASK_ID, requirement_id: REQUIREMENT_ID, generation: 1, registered_ref: `asset_${A}`, receipt_digest: A,
    }] }, "stale_generation"],
    ["cross task", { assets: [{
      binding_id: "asset_binding", source: "geo_gse", input_requirement_ref: "input_geo",
      task_id: "other_task", requirement_id: REQUIREMENT_ID, generation: GENERATION, registered_ref: `asset_${A}`, receipt_digest: A,
    }] }, "cross_task_binding"],
    ["build mismatch", { assets: [{
      binding_id: "asset_binding", source: "geo_gse", input_requirement_ref: "input_geo",
      task_id: TASK_ID, requirement_id: "other_build", generation: GENERATION, registered_ref: `asset_${A}`, receipt_digest: A,
    }] }, "requirement_mismatch"],
  ] as const)("fails closed on %s", async (_label, overrides, code) => {
    const base = await fixture(overrides);
    const error = await capture(() => resolveDatasetExecutionProposal2(base.proposal, base.context));
  ...
  test("rejects ambiguous asset/result and duplicate records", async () => {
    const base = await fixture({
      results: [{
        binding_id: "asset_binding", source: "geo_gse", input_requirement_ref: "input_geo",
        task_id: TASK_ID, requirement_id: REQUIREMENT_ID, generation: GENERATION, registered_ref: "result_ambiguous", receipt_digest: B,
      }, baseResult()],
    });
    const ambiguous = await capture(() => resolveDatasetExecutionProposal2(base.proposal, base.context));
    expect(ambiguous.code).toBe("ambiguous_binding");

    const duplicate = await fixture({
      assets: [baseAsset(), baseAsset()],
    });
    expect((await capture(() => resolveDatasetExecutionProposal2(duplicate.proposal, duplicate.context))).code)
  ...
  test("requires activated, non-example exact capabilities and verified family digest", async () => {
    ...
    expect((await capture(() => resolveDatasetExecutionProposal2(revoked.proposal, revoked.context))).code)
      .toBe("family_revoked");
    ...
    expect((await capture(() => resolveDatasetExecutionProposal2(exampleProposal, example.context))).code)
    ...
    expect((await capture(() => resolveDatasetExecutionProposal2(badFamily.proposal, bad))).code)
  ...
  test("rejects proposal/resolved hybrids, accessors, and proxies", async () => {
    ...
    expect((await capture(() => resolveDatasetExecutionProposal2(hybrid, base.context))).code)
      .toBe("invalid_proposal");

    let reads = 0;
    const accessor = { ...base.proposal };
    Object.defineProperty(accessor, "requirement_id", {
      enumerable: true,
      get() {
        reads += 1;
    ...
    expect((await capture(() => resolveDatasetExecutionProposal2(accessor, base.context))).code)
      .toBe("invalid_proposal");
    expect(reads).toBe(0);

    const proxy = new Proxy(base.proposal, { get() { reads += 1; return undefined; } });
    expect((await capture(() => resolveDatasetExecutionProposal2(proxy, base.context))).code)
      .toBe("invalid_proposal");
    expect(reads).toBe(0);
  });
  ...
  test("round-trips through the resolved parser and stable canonical bytes", async () => {
    const base = await fixture();
    const result = await resolveDatasetExecutionProposal2(base.proposal, base.context);
    const wire = JSON.parse(stableStringify(result.resolved)) as unknown;
    expect(parseResolvedDatasetExecutionSpec2(wire, "$resolved")).toEqual(result.resolved);
    expect(parseResolvedDatasetExecutionSpec2(wire, "$resolved")).toEqual(result.resolved);

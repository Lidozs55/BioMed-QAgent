...
describe("createBrowserFamilyTableOutputs", () => {
  ...
  it("rejects a table without source evidence", async () => {
    await expect(createBrowserFamilyTableOutputs({ taskId: "task", buildId: "build", evidenceRoot: await mkdtemp(path.join(os.tmpdir(), "browser-evidence-")), implementationDigest: "b".repeat(64), integratedTables: { one: result("integrated_one", "integrated_table") }, sourceEvidenceByTable: {} })).rejects.toThrow("missing source evidence");
  });
});

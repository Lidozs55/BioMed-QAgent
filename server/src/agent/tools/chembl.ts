  ...
  return [
    {
      ...
      parameters: {
        ...
        properties: {
          query: {
            type: "string",
            description: "Search keyword (e.g. 'aspirin', 'EGFR inhibitor', 'kinase').",
          },
          max_results: {
            type: "integer",
            default: 20,
            description: "Maximum number of molecule records to return (default 20).",
          },
        },
        required: ["query"],
      ...
      execute: async (argumentsValue, signal) => {
        ...
        const maxResults =
          ...
          record.max_results > 0
            ? record.max_results
            : 20;
        const result = await searchChembl(record.query, maxResults, {
          client,

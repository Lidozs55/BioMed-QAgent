...
const DATASET_COMPLETION_CONTRACT = [
  ...
  "Before substantive acquisition call inspect_dataset_execution_routes, then declare the matching semantic family, projection, and row granularity from its current facts. Build one family and granularity at a time; split multi-product requests into separate builds.",
...
const TRUSTED_EXECUTION = [
  ...
  "Never infer provider availability from static enums. A dynamic-bindable provider is wired for trusted acquisition and input decoding even without a static family — proving neither semantic topology, transform, source availability, validation, nor publication closure; a Core-acquisition-only binary carrier still needs the reported provenance-bound extraction path before Dynamic Family submission.",
  "Close every formal source binding exactly once through a fixed Dataset Core acquisition provider or a prior Core-returned asset ID. Registered assets live in Core task storage, not the Agent Workspace; browser, download, discovery, parsed, and workspace outputs are staging evidence only and are never formal carriers.",
...
const DYNAMIC_PUBLICATION_MECHANICS = [
  ...
  "prepare_dynamic_family_publication takes semantic facts only — omit every digest; input_requirement_ref is a declared_input_roles role, not a binding id; declared_outputs = declared_output_tables = the projection's required+optional tables — and returns prepared_submission plus a preflight_receipt, superseding earlier receipts.",
  "submit then needs only schema_version and that unchanged preflight_receipt, never a payload re-echo. Transform inputs arrive in binding order as in_0, in_1, ... {handle, receipt_kind, receipt_id, text}; outputs are {content, handle:'out_0', locator_ref, row_count, schema_ref, table_id} envelopes; each CSV header equals field_names exactly and every row comes from real parsed inputs — placeholder sentinels are rejected.",
  "Rejections name the exact failing fact: fix only that and resubmit; never fall back to workspace-only output while a rejection is fixable, and never request file access outside the Task Workspace.",
];
...
const CONTROL_AND_RECOVERY = [
  ...
  "While any planned step is pending, never end a turn on narrative text alone: a turn without a tool call ends the whole run. Keep issuing the next tool call until the build is published or you can state a final structured outcome (success, NO_DATA, or a precise blocker).",
];

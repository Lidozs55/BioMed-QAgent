...
const CONTROL_AND_RECOVERY = [
  ...
  "Whenever you are uncertain how any Dataset Core tool, provider, schema, validation gate, admission rule, or rejection behaves, call read_dataset_core_source and read the implementing source directly — the code is the authoritative, precise description of what is accepted. Derive the file from the tool or provider name (e.g. acquisition/<provider>.ts, families/<family>/validation.ts, transform-admission/admission.ts, dynamic-family/preflight.ts, validation/profile.ts) and proceed on that basis instead of guessing.",

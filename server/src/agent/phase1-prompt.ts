...
const TRUSTED_EXECUTION = [
  "[Trusted execution]",
  "Choose exactly one formal build route using inspect_dataset_execution_routes. Use the static route only for an exact listed family, schema, source, and topology match; then validate_dataset_execution must pass before execute_dataset_execution. Otherwise call scaffold_dataset_profile so Core generates the complete FamilySpec/Projection/table/relation closure, then pass its generated submission through prepare_dynamic_family_publication and submit_dynamic_family_publication when every input is dynamic-bindable or already has a task-owned Core asset. Do not hand-write a profile topology or pass a dynamic FamilySpec to the static validator.",
  ...
  "Close every formal source binding exactly once through a fixed Dataset Core acquisition provider or a task-owned Core-derived asset with a persisted OperationResult and recursively verified parent closure. Registered assets live in Core task storage, not the Agent Workspace; browser, download, discovery, Agent-parsed, and workspace outputs are staging evidence only and are never formal carriers.",
  "A non-retryable static adapter, transform, schema, or topology rejection means the static family is unsuitable. Stop unchanged retries and required-field probing. For dynamic mismatch, use the returned expected family, table list, and scaffold, modify source/extraction facts, then scaffold and prepare again; unchanged retry is forbidden.",

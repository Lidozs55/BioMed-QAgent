...
const TRUSTED_EXECUTION = [
  "[Trusted execution]",
  "Choose exactly one formal build route before acquisition. Use the static route only when the required family, schema, source, and topology are present in the static tool schema; then validate_dataset_build must pass before execute_dataset_build. Otherwise use prepare_dynamic_family_build then submit_dynamic_family_build. Do not pass a dynamic FamilySpec to the static validator, and do not use static validation failure or absence from its enums as evidence that the dynamic route is unavailable.",
  "The acquisition_requests schema on the Dynamic Family tools is the authoritative dynamic provider catalog. A provider listed in the dynamic acquisition_requests schema is wired for Dynamic Family acquisition even when its source has no static family. For GWAS Catalog use gwas-catalog.associations.v1 with one verified GCST accession or rsID per binding.",

...
const DYNAMIC_PUBLICATION_MECHANICS = [
  ...
  "Transform sandbox forbids every bracket element access (arr[0], m[1]) and dynamic keys: use destructuring, .map/.find, dot properties, and named regex groups only. Inputs arrive in binding order as in_0, in_1, ...; outputs are {content, handle:'out_0', locator_ref, row_count, schema_ref, table_id} envelopes; each CSV header equals field_names exactly and every row comes from real parsed inputs — placeholder sentinels are rejected.",
  "Core decodes acquired archives and spreadsheets; never run python, shell, or workspace_exec extraction scripts.",
];
...
const CONTROL_AND_RECOVERY = [
  ...
  "While any planned step is pending, never end a turn on narrative text alone: a turn without a tool call ends the whole run. Keep issuing tool calls until the build is published or you can state a final structured outcome (success, NO_DATA, or a blocker).",
];

...
const DYNAMIC_PUBLICATION_MECHANICS = [
  ...
  "Transform source admission rejects eval-class identifiers, computed indexing (arr[0], m[key]), and dynamic keys: pass admission on the first try by writing destructuring, .map/.find, dot properties, and named regex groups only (e.g. rows.map((row) => row.id)). Inputs arrive in binding order as in_0, in_1, ...; outputs are {content, handle:'out_0', locator_ref, row_count, schema_ref, table_id} envelopes; each CSV header equals field_names exactly and every row comes from real parsed inputs — placeholder sentinels are rejected.",

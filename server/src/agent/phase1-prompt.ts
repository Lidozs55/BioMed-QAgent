...
export const PHASE1_SYSTEM_PROMPT = [
  ...
  "A non-retryable static adapter or transform rejection means the registered static family cannot satisfy this request. Stop retrying execute_dataset_build and never probe schema vocabulary by varying required_fields. Switch immediately to prepare_dynamic_family_build, bind build_proposal.transform_refs[0].digest to its host_descriptor_digest, then pass the exact task/build/generation-bound preflight_receipt unchanged to submit_dynamic_family_build; the receipt is the authoritative FamilySpec/Projection/TypeScript-transform structural proof and no failure-driven digest handshake is permitted. Use dynamic execution whenever the user requires an exact multi-table topology or raw-field retention beyond the registered static family.",

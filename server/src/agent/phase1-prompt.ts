...
const DYNAMIC_PUBLICATION_MECHANICS = [
  ...
  "Transform source admission rejects eval-class identifiers and EVERY bracket access. Use destructuring, .at(), .shift(), dot paths, and JSON.parse; admission reports all violations with line:column. Inputs are in_0, in_1, ... in the order of the source_bindings whose binding_kind is transform_input; provenance_only bindings never receive a Host input. Outputs are {content,handle:'out_0',locator_ref,row_count,schema_ref,table_id}. CSV headers must equal field_names, rows must come from inputs, and placeholder sentinels are rejected.",
  "binary_archive assets: preview_core_asset lists members and extract_core_archive registers a decoded member as a new bindable asset; never run python, shell, or workspace_exec extraction. For supplementary evidence, pass the extracted member asset to governed extraction, then bind the reviewed text/JSON carrier as transform_input and the binary member as provenance_only so both enter formal provenance without exposing binary bytes to the Transform Host.",
];
